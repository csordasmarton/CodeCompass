...
function (array, declare, lang, domClass, dom, Memory, Observable, topic, Tree,
  ObjectStoreModel, urlHandler, model, util, viewHandler) {
  ...
  var BrowsingHistory = declare(HtmlTree, {
    constructor : function () {
      var that = this;
    ...
    onClick : function (item, node, event) {
      this._blockHashchangeEvent = true;

      this._stack.getCurrent().setSelected(false);
      this._stack.push(node);

      if (isOtherBranch(this, node))
        this._currentLeaf = node;

      var state = domClass.contains(event.target, 'jumpfrom')
        ? item.fromUrl
        : item.toUrl;

      urlHandler.setState(state);
      topic.publish('codecompass/initByUrl', urlHandler.getState());
    },
  ...
  viewHandler.registerModule(browsingHistory, {
